...
function displayCart() {
    ...
    // Show cart items container if there's at least one item
    // cartItemsContainer.style.display = cart.length > 0 ? "block" : "none";
...
document.querySelectorAll('.addcart').forEach(button => {
    button.addEventListener('click', () => {
        const productId = button.getAttribute('data-id');
        const productName = button.getAttribute('data-name');
        const productPrice = parseFloat(button.getAttribute('data-price')); // Convert price to number
        const productImage = button.getAttribute('data-image');

        addToCart(productId, productName, productPrice, productImage, button); // Pass button reference
    });
});
